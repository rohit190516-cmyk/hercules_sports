...
const WhyChooseSection = () => {
  ...
  return (
    <section id="why-us" className="py-24 md:py-32 bg-white relative" aria-label="Why choose Hercules Sports">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <motion.div
              ...
              className="p-8 border border-border bg-slate-50/50 rounded-xl transition-all duration-300 group hover:shadow-md hover:-translate-y-1 hover:bg-white"
            >
              <f.icon className="w-8 h-8 text-primary mb-4 group-hover:scale-110 transition-transform" />
              <h3 className="text-base font-bold mb-2 font-display tracking-wide text-foreground">
                {f.title}
              </h3>
              <p className="text-sm font-body leading-relaxed text-muted-foreground">{f.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
